...
voff_file = '/home/pi/vac/VacuumSystem/voff_time.txt'
von_file  = '/home/pi/vac/VacuumSystem/von_time.txt'
vac_on    = 0
vacuum_on_time_sec    = 10
vacuum_off_time_sec   = 10 
...
GPIO_BALL_CLOSE   = 11  # GREEN
GPIO_PRESSURE_VALVE = 7 # GREY 
GPIO_BOTH_OPEN = 7 # GREY 
GPIO_QUIT = 18 
GPIO_SHUTDOWN = 16 
...
#
# Open both valves:
#  - Deassert GPIO which closes Ball Valve
#  - Assert GPIO which opens Ball Valve and Vacuum Valve
#
def turn_on_vacuum ():
    ...
    # Deassert GPIO which closes Ball Valve
    GPIO.output ( GPIO_BALL_CLOSE, False ) 

    # Open valves to vacuum bag.
    GPIO.output ( GPIO_BOTH_OPEN, True ) 
...
#
# Close both valves
#  - Dessert GPIO which opens Ball Valve and Vacuum Valve
#  - Assert GPIO which closes Ball Valve
#
def turn_off_vacuum ():
    ...
    # Deassert the GPIO which opens the High Pressure Valve
    GPIO.output ( GPIO_BOTH_OPEN,  False )

    # Close Ball valve
    GPIO.output ( GPIO_BALL_CLOSE, True ) 
...
def regulate_vacuum_sense ():
    ...
    while keep_vacuum :
        ...
        if ( vac_volts < VAC_V_MIN ) :
            if ( vac_on == 1 ) :
                print "INFO: turn off vacuum: psi = ", vac_volts_to_psi (vac_volts), ", volts = ", vac_volts
                turn_off_vacuum ()
            #else:
                #print "INFO: Vac voltage indicates sufficient vacuum but Vacuum is not on so do not turn off."
        elif ( vac_volts > VAC_V_MAX ) :
            if ( vac_on == 0 ) :
                print "INFO: turn on vacuum: psi = ", vac_volts_to_psi (vac_volts), ", volts = ", vac_volts
                turn_on_vacuum ()
            #else:
                #print "INFO: Vac voltage indicates vacuum needs to increase but Vacuum is already on so do not turn ON."
